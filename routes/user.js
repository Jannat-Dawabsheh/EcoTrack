...
router.post('/signup', async (req, res) => {
    ...
    const emailExists = await signupValidation.checkEmailExists(email,con);
    ...
    const usernameExists = await signupValidation.checkUsernameExists(name,con);
...
router.get('/sensors', (req, res) => {
  ...
  router.post('/dataCollection', CheckAuth, async (req, res) => {
    const userId = req.userId;
    const { interestID, value } = req.body;
  
    
    con.query('INSERT INTO data_collection (interestID, value, date, time) VALUES (?, ?, CURDATE(), CURTIME())', [interestID, value], async (err, results) => {
      if (err) {
        console.error('MySQL query error:', err);
        return res.status(500).json({ error: 'Internal Server Error', details: err.message });
      }
  
      await score(con ,userId, 10);
